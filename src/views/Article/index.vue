<template>
  <div class="article-container">
    ...
    <van-popup
      ...
      :style="{ height: '20%' }"
    >
      <!-- v-if加上就是每次关闭 组件销毁   每次打开，组件重新创建mounted就会再次执行一次 -->
      <AddComment
        v-if="addCommentShow"
        :target="article_id"
        @add-comment="
    ...
    <van-popup position="bottom" v-model="idReplayShow" style="height: 100%">
      <ReplayComment
        :comment="comment"
        @close="idReplayShow = false"
        v-if="idReplayShow"
      ></ReplayComment>
    </van-popup>
  </div>
...
import ArticleComment from './components/ArticleCommen'
